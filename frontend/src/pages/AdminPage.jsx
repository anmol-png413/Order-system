...
import { Plus, Edit2, Trash2, X, BarChart3, Package, Users, ShoppingBag, Upload, Eye, EyeOff } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const [productForm, setProductForm] = useState({ name: '', price: '', category: '', description: '', isAvailable: true });
  ...
  const openProductModal = (product = null) => {
    ...
    setProductForm(product
      ? { name: product.name, price: product.price, category: product.category, description: product.description || '', isAvailable: product.isAvailable }
      : { name: '', price: '', category: '', description: '', isAvailable: true });
    setImageFile(null);
        ...
        {tab === 'Dashboard' && stats && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { label: 'Total Orders', value: stats.today.total, icon: ShoppingBag, color: 'orange' },
                { label: 'Pending', value: stats.today.pending, icon: Package, color: 'yellow' },
                { label: 'In Progress', value: stats.today.inProgress, icon: Package, color: 'blue' },
                { label: 'Completed', value: stats.today.completed, icon: Package, color: 'green' },
              ].map(s => (
                <div key={s.label} className="card p-5">
                  <p className="text-zinc-500 text-xs uppercase tracking-wide mb-3">{s.label}</p>
                  <p className="text-4xl font-extrabold text-white" style={{ fontFamily: 'Sora, sans-serif' }}>{s.value}</p>
                </div>
              ))}
            </div>
            <div className="card p-6">
              <p className="text-zinc-500 text-sm mb-2">Revenue Today (Completed Orders)</p>
              <p className="text-5xl font-extrabold text-orange-400" style={{ fontFamily: 'Sora, sans-serif' }}>
                  ...
                  <div key={p._id} className={`card overflow-hidden ${!p.isAvailable ? 'opacity-50' : ''}`}>
                    ...
                    <div className="p-4">
                      <div className="flex items-start justify-between gap-2 mb-1">
                        <h3 className="font-semibold text-zinc-200 text-sm leading-tight" style={{ fontFamily: 'Sora, sans-serif' }}>
                          {p.name}
                        </h3>
                        {!p.isAvailable && <span className="text-xs text-zinc-600 flex-shrink-0">Hidden</span>}
                      </div>
                      <p className="text-xs text-zinc-600 mb-2">{p.category}</p>
                      <p className="text-orange-400 font-bold">₹{p.price.toFixed(2)}</p>
                      <div className="flex gap-2 mt-3">
                        ...
                        <button onClick={() => deleteProduct(p._id)}
                          className="flex-1 bg-red-500/10 hover:bg-red-500/20 text-red-400 text-xs font-medium py-2 rounded-xl transition-colors flex items-center justify-center gap-1">
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-start justify-center p-4 overflow-y-auto">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 w-full max-w-md my-8 shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setProductModal(false)} className="text-zinc-500 hover:text-zinc-300">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-3">
              {[
                ...
                { label: 'Category', key: 'category', placeholder: 'e.g. Burgers, Drinks...' },
                ...
                <div key={f.key}>
                  ...
                  <input type={f.type || 'text'}
                    ...
                    className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 text-zinc-100 placeholder-zinc-600 focus:outline-none focus:border-orange-500 text-sm" />
                </div>
              ))}
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={() => setProductModal(false)} className="flex-1 btn-ghost">Cancel</button>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setUserModal(false)} className="text-zinc-500 hover:text-zinc-300">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="relative">
                  <input type={showPass ? 'text' : 'password'} value={userForm.password}
                    ...
                    className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 pr-10 text-zinc-100 placeholder-zinc-600 focus:outline-none focus:border-orange-500 text-sm" />
              ...
              <div>
                ...
                <select value={userForm.role}
                  ...
                  className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 text-zinc-100 focus:outline-none focus:border-orange-500 text-sm">
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={() => setUserModal(false)} className="flex-1 btn-ghost">Cancel</button>
              <button onClick={saveUser} className="flex-1 btn-primary">Create User</button>
            </div>
          </div>
        </div>
      )}
