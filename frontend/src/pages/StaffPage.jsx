...
function fmtQty(quantity, quantityLabel, unit) {
  if (quantityLabel) {
    if (unit !== 'piece' && quantity > 1) return `${quantityLabel} × ${quantity} Box`;
...
function buildCustomerSlipHTML(tokenNumber, items, notes, discountPercent, bulkInfo) {
  const now = new Date();
  const total = items.reduce((s, i) => s + i.price * i.quantity, 0);
...
function buildInternalReceiptHTML(tokenNumber, items, notes, discountPercent, bulkInfo) {
  const now = new Date();
  const total = items.reduce((s, i) => s + i.price * i.quantity, 0);
  const discount = +(total * (discountPercent / 100)).toFixed(2);
  const payable = +(total - discount).toFixed(2);
      ...
      <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-sm overflow-hidden animate-slide-up" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800 bg-purple-500/10">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-white text-base" style={{ fontFamily: 'Sora, sans-serif' }}>Bulk Order</h3>
              <p className="text-xs text-zinc-500">Advance booking details</p>
            </div>
          </div>
          <button onClick={onClose} className="w-8 h-8 bg-zinc-800 hover:bg-zinc-700 rounded-lg flex items-center justify-center text-zinc-400">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <label className="flex items-center gap-1.5 text-xs font-semibold text-zinc-400 uppercase tracking-wide mb-2">
              <Calendar className="w-3 h-3" /> Pickup / Delivery Schedule
            </label>
            <input
              type="text"
              value={bulkSchedule}
              onChange={e => setBulkSchedule(e.target.value)}
              placeholder="e.g. 3 Nov 2025, Evening"
              className="w-full bg-zinc-800 border border-zinc-700 focus:border-purple-500 rounded-xl px-4 py-3 text-zinc-100 placeholder-zinc-600 text-sm focus:outline-none transition-colors"
            />
          </div>
          ...
          {/* Summary card */}
          {advance > 0 && (
            <div className="bg-zinc-800/60 rounded-xl p-4 space-y-2 border border-zinc-700">
    ...
    <div className="fixed inset-0 z-50 bg-black/90 flex flex-col">
      <div className={`flex items-center justify-between px-4 py-4 border-b border-zinc-800 ${isPacking ? 'bg-blue-500/10' : 'bg-green-500/10'}`}>
        <div className="flex items-center gap-3">
          {isPacking ? <Package className="w-6 h-6 text-blue-400" /> : <Bell className="w-6 h-6 text-green-400" />}
            ...
            {filtered.map(order => (
              <div key={order._id} className={`rounded-2xl border overflow-hidden ${
                ...
                <div className="p-4">
                  ...
                  {/* Bulk badge */}
                  {order.bulk?.phone && (
                    <div className="flex items-center gap-1.5 mb-2 bg-purple-500/10 border border-purple-500/20 rounded-lg px-2 py-1">
...
export default function StaffPage() {
  ...
  async function placeOrder() {
    ...
    try {
      ...
      // No auto-close — user manually dismisses via buttons
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex flex-col">
      ...
      {/* ── STATUS MODAL ── */}
      {statusModal && (
        <StatusModal type={statusModal} orders={liveOrders} onClose={() => setStatusModal(null)} onDelete={deleteOrder} deleting={deleting} />
      )}
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-sm flex flex-col animate-slide-up relative" onClick={e => e.stopPropagation()}>
            ...
            <div className="w-full bg-zinc-800 flex-shrink-0">
              <img src={modalProduct.image || IMG_FALLBACK} alt={modalProduct.name}
                className="w-full object-contain" style={{ maxHeight: '180px' }}
                onError={e => { e.target.src = IMG_FALLBACK; }} />
            </div>
                ...
                return (
                  <button key={product._id} onClick={() => openModal(product)}
                    ...
                    <div className="aspect-square overflow-hidden bg-zinc-800 relative">
                      <img src={product.image || IMG_FALLBACK} alt={product.name} loading="lazy" width="200" height="200"
...
//       <div className="overflow-y-auto p-3 space-y-2 flex-1 min-h-0" style={{ scrollbarWidth: 'none' }}>
//         {cart.length === 0 ? (
//           <div className="flex flex-col items-center justify-center h-full text-zinc-600 py-10">
//             <ShoppingCart className="w-12 h-12 mb-3 opacity-20" />
//             <p className="text-sm">Tap products to add</p>
...
//       {/* ── Fixed bottom: notes + discount + bulk + place order ── */}
//       {cart.length > 0 && (
//         <div className="flex-shrink-0 border-t border-zinc-800 overflow-y-auto" style={{ maxHeight: '55vh', scrollbarWidth: 'none' }}>
...
//         {/* RIGHT — Cart (desktop) */}
//         <div className="hidden sm:flex w-72 lg:w-80 flex-shrink-0 flex-col card overflow-hidden" style={{ height: 'calc(100vh - 130px)', position: 'sticky', top: '16px' }}>
